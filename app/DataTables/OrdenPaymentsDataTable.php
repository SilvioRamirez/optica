...
class OrdenPaymentsDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                if (auth()->user()->can('orden-payment-show')) {
                    $buttons .= '<a href="' . route('orden-payments.show', $query->id) . '" class="btn btn-info btn-sm" title="Mostrar"><i class="fa fa-eye"></i></a>';
                }

                if (auth()->user()->can('orden-payment-delete')) {
            ...
            ->filterColumn('monto', function ($query, $keyword) {
                $query->where('monto', 'like', "%$keyword%");
            })
            ->addColumn('tipo_id', function ($query) {
                return $query->tipo ? $query->tipo->tipo : 'N/A';
            })
            ->filterColumn('tipo_id', function ($query, $keyword) {
            ...
            ->addColumn('image_path', function ($query) {
                return $query->image_path ? '<div class="d-flex align-items-center gap-2"><img src="' . $query->image_path . '" class="img-fluid" style="width: 50px; height: 50px;"> <a href="' . $query->image_path . '" target="_blank" class="btn btn-primary btn-sm"><i class="fa fa-magnifying-glass-plus"></i></a></div>' : '';
            })
            ->rawColumns(['action', 'numero_orden', 'tipo_id', 'image_path', 'origen_id'])
            ->setRowId('id');
    }
    ...
    {
        return $this->builder()
            ->setTableId('orden-payments-table')
    ...
    {
        return 'Ordenes_Pagos_' . date('YmdHis');
    }
}
