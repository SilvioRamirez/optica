...
class ClientePaymentsDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($query) {
                ...
                if (auth()->user()->can('cliente-payment-show')) {
                ...
                if (auth()->user()->can('cliente-payment-edit')) {
            ...
            ->addColumn('cliente', function ($query) {
                return $query->cliente ? $query->cliente->name : 'N/A';
            })
            ...
            ->rawColumns(['action', 'cliente', 'file'])
            ->setRowId('id');
    }
     ...
     */
    public function query(ClientePayment $model): QueryBuilder
    {
        return $model->newQuery()->with('cliente'); //Se reduce de 82 consultas a 9 con los with
    }
    ...
    {
        return $this->builder()
            ->setTableId('cliente-payments-table')
    ...
    public function getColumns(): array
    {
        ...
        $columns[] = Column::make('cliente_id')->title('Cliente ID');
        $columns[] = Column::make('cliente')->title('Cliente');
    ...
    {
        return 'ClientePayments_' . date('YmdHis');
    }
}
